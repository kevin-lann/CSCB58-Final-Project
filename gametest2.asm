...
LOOPU:	# MOVE UP >>>>>>>>>>>>>>>
	...
	subi $t9, $t9, WIDTH	# we need to measure from the topmost row
	move $a0, $t9
	addi $a0, $a0, 4	# have to make sure we are checking the rightmost edge
	
	jal HitTopBorder
	
	beq $v0, $zero, NOSWITCHU
	j LOOPD			# switch to moving down if we hit top side
NOSWITCHU:
	j LOOPU			# cont loop if didnt hit bottom side yet
...
LOOPD:	# MOVE DOWN <<<<<<<<<<<<<<<<
	...
	beq $v0, $zero, NOSWITCHD
	j LOOPU			# switch to moving up if we hit bottom side
NOSWITCHD:
	j LOOPD			# cont loop if didnt hit bottom side yet
